...
FIXED_SAFE_RUNNER = ROOT / "tools" / "run_safe_goal_checks_20260620_ai_default_probe_20260622.py"
...
def find_rks_runtime_status(scenario: str, rks_rows: list[dict[str, Any]]) -> str:
    for row in rks_rows:
        row_scenario = str(row["scenario"])
        if scenario == row_scenario:
            return str(row.get("runtime_intake_status", ""))
        if scenario in split_grouped_scenarios(row_scenario):
            return str(row.get("runtime_intake_status", ""))
    return ""
...
def classify_bundle(
    ...
    if "RKS" in status or has_real_rks_matrix_status or scenario in {"38", "42", "51/52", "56", "57", "58"}:
        if scenario in {"57", "58"} and rks_runtime_status == "RKS_RUNTIME_EVIDENCE_CONFIRMED":
            return (
                "RK10_EDITOR_RUNTIME_BUNDLE",
                "RK10操作担当者",
                "RKS open/build/runtime/latest logは確認済み。残りは最終証跡入力または業務費用確認",
            )
        return (
            "RK10_EDITOR_RUNTIME_BUNDLE",
            "RK10操作担当者",
...
def classify_resolution_lane(
    ...
    if (
        ...
        and scenario in {"57", "58"}
        and rks_runtime_status == "RKS_RUNTIME_EVIDENCE_CONFIRMED"
    ):
        return (
            "NEEDS_FINAL_OPERATOR_EVIDENCE",
            "RKS open/build/runtime/latest logは確認済み。残る業務費用または最終証跡入力を確認する",
            "RK10 GUI再確認は不要化し、不可逆な本番書込・実メールだけ別承認に残す",
        )
...
def build_rows(
    ...
    for item in snapshot_payload["scenarios"]:
        ...
        rks_runtime_status = find_rks_runtime_status(scenario, rks_rows)
        bundle, owner, blocked_until = classify_bundle(
            scenario,
            status,
            rks_status,
            rks_runtime_status,
