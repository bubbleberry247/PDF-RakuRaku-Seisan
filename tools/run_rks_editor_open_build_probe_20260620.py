...
The probe opens an RKS in RK10 editor, accepts the development-license dialog
by pressing OK without changing the default radio selection, waits for the
editor window, reconciles Debugger artifacts, and closes only new RK10 editor
processes it started. It never presses ButtonRun and never executes business
operations.
"""
...
def run_metadata_guard(rks_path: Path, out_dir: Path, scenario: str) -> int:
    ...
    stdout_path = out_dir / f"s{scenario}_metadata_guard.stdout.txt"
    stderr_path = out_dir / f"s{scenario}_metadata_guard.stderr.txt"
    command = [
        sys.executable,
...
def write_top_window_details(
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    detail_path = out_dir / f"s{target.scenario}_top_window_details.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--scenario", action="append", default=["58"])
    parser.add_argument("--hold-seconds", type=int, default=15)
    parser.add_argument("--launch-mode", choices=["startfile", "exe"], default="startfile")
...
def main() -> int:
    args = parse_args()
    scenarios = {str(scenario) for scenario in args.scenario}
