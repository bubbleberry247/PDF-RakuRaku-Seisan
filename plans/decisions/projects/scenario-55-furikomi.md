...
## 2026-02-10: [CODE] 5ヶ月精度監査 + P0正規化バグ修正

### 精度監査結果（チーム編成: Investigator + Reproducer + Devil's Advocate + Reviewer）

**5ヶ月再現テスト（R7.7〜R7.11）**:

| 月 | GT | RPA | 金額一致 | A列 | E列 | 完全一致 | 再現率 |
|----|-----|-----|---------|-----|-----|---------|--------|
| R7.7 | 30 | 27 | 96% | 93% | 85% | 21/27 | 80% |
| R7.8 | 23 | 21 | 95% | 90% | 95% | 18/21 | 87% |
| R7.9 | 37 | 33 | 97% | 100% | 100% | 32/33 | 97% |
| R7.10 | 33 | 26 | 96% | 100% | 100% | 25/26 | 97% |
| R7.11 | 28 | 23 | 91% | 91% | 91% | 19/23 | 86% |

**6件不一致の根本原因（コードバグ=0件）**:
1. 山庄㈱(R7.9): 月跨ぎ（支払日=9月、申請日=10月）
2. ㈱ワークマン(R7.10): 月跨ぎ（吉田さんが42,200を11月→12月へ繰延）
3. 安城印刷㈱(R7.11): フィルタ差（都度振込255,750をRPAが正しく除外）
4. ㈱ワークマン(R7.11): #2の対称（42,200が12月に計上）
5. 名古屋フランスCorp㈱(R7.11): **正規化バグ** → P0修正実施
6. 太田商事㈱(R7.11): 新規業者（テンプレート未登録は正常）

### P0修正: variant set intersection

**問題**: `_get_template_from()` がCSV名のバリアントのみ生成し、テンプレートキーのバリアントは生成しなかった。
- CSV: `"名古屋フランス corp㈱"` → no_space = `"名古屋フランスcorp㈱"`
- Template: `"名古屋フランスCorp㈱"` → lowered = `"名古屋フランスcorp㈱"`
- 交差するがマッチしなかった。

**修正**: exact match → **variant set intersection（新Stage 2）** → substring containment の3段階に変更。
テンプレートキーにも`_create_matching_variants()`を適用し、CSV側バリアントとの集合交差でマッチング。

**検証**: `"名古屋フランス corp株式会社"` → FOUND (A=6799, E=接待交際費)

### 変更ファイル
- `C:\ProgramData\RK10\Robots\55 １５日２５日末日振込エクセル作成\tools\excel_writer_v2.py`
  - `_get_template_from()`: Stage 2 (variant set intersection) 追加
  - `_index_has_supplier()`: 同上
...
| 指摘 | 対応 |
|------|------|
| P0: 名古屋フランス正規化バグ | **修正済み** |
| P0: GT抽出の〃未解決 | テスト手法の問題（本番に影響なし）。将来改善 |
| HIGH: 再現率膨張 | 認識済み。精度天井を90-95%と明示 |
| HIGH: 循環論証 | 認識済み。テンプレートは運用データから構築される性質 |
| HIGH: E列の根本限界 | 1業者=複数科目のケースは手動修正前提 |

## 2026-02-10: 実書込テスト結果と発見事項

### [TEST] source-excel モード実書込テスト（R7.10末日データ）

**条件**: R7.10末日 cc300 CSV → 原本コピー → RPA_R8.1月分シート作成 → --reset-amounts付き

| 項目 | 結果 |
|------|------|
| 書込成功 | 31/31件（登録済28 + 未登録3） |
| 処理金額合計 | 2,385,878円 |
| I列小計（25日） | 0円（25日データなし） |
| I列小計（末日） | 2,385,878円 |
| コードバグ | 0件 |

### [DISCOVERY] H列（請求額）は数式列 — I列から自動計算

H列の全セルは数式:
- 奇数行（本体）: `=ROUNDDOWN(I17-H18, 0)` （税抜額 = 支払額 - 税額）
- 偶数行（税）: `=ROUNDDOWN(I17*10/110, 0)` （税額 = 支払額×10/110）

`--reset-amounts`でI列を0にすると、H列も自動的に0に再計算される。
RPA書込後のH値が原本と異なるのは、**書込データが異なる月のためであり正常動作**。
RPAはH列に一切書き込んでいない（I列とJ列のみ）。

### [DISCOVERY] 川原こづえのcc300 CSV未含有問題

| データソース | 川原こづえ |
|-------------|-----------|
| R7.10 UNVERIFIED（フィルタなし） | 4件あり（総合振込3 + 都度振込1） |
| R7.10 cc300（会社コード300フィルタ済み） | **0件 — 取得漏れ** |
| R7.11 cc300 | あり（12月データ） |
| テンプレートマスタ | 登録済み（司法書士　川原こずえ / account=管理諸費） |

**原因**: 11月のcc300スクレイピングで取得漏れ（ページングや日付フィルタのタイミング差の可能性）。
12月データでは正常に取得されているため、恒常的なバグではなく、スクレイピングタイミングに起因する散発的な問題。

### [CONFIRMED] 前払/当月分の区分廃止の妥当性を具体例で確認

**川原こずえのケース**:
- 請求書発行: 2025/11/06
- 楽楽精算申請: 2025/11/07
- 支払予定日: 2025/12/30（12月末日）

→ 会計的には「前月分（11月発生）→ 当月末（12月）支払い」= 未払費用
→ しかし原本では「当月分当月末支払」（Section 4）に記入 — 吉田さんの手動判断の揺らぎ

**結論**: この区分の曖昧さが廃止理由そのもの。全件「未払費用」統一は正しい判断。
RPAはSection 3/4を再現する必要なし（ヒアリング12/8, 1/27確認済み）。

## 2026-02-10: [RULE] 月ズレ（会計月オフセット）ルール確定

### 根本原因: R7.12月分 ≠ 12月CSV

| 項目 | 値 |
|------|-----|
| R7.12月分シート（原本） | 25日: 4件/¥163,938、末日: 26件/¥2,597,774 |
| RPA出力（12月CSV使用） | 25日: 3件/¥132,142、末日: 21件/¥977,610 |
| 乖離率 | 25日: 81% → 末日: 38% |

**根本原因**: R7.12月分 = 「12月**会計期**」= **1月に支払うデータ**。12月CSVは12月に支払うデータ = R7.**11**月分に対応。

### 月ズレの公式（SSOT）

```
シート名    = R{和暦}.{会計月}月分
支払日      = 会計月+1 の 15日/25日/末日
CSV名       = payment_{YYYYMM}_{suffix}.csv  ※YYYYMMは支払日の暦月
```

| シート名 | 対応する支払月 | 必要なCSV |
|----------|--------------|-----------|
| R7.10月分 | 2025年11月支払 | payment_202511_*.csv |
| R7.11月分 | 2025年12月支払 | payment_202512_*.csv |
| R7.12月分 | 2026年1月支払 | payment_202601_*.csv |

**コードの`payment_date.month - 1`ロジックは正しい。問題はテスト時のCSV選択ミス。**

### [CODE] 名前マッチング修正2件

**修正ファイル**: `C:\ProgramData\RK10\Robots\55 １５日２５日末日振込エクセル作成\tools\excel_writer_v2.py`

1. **株式会社↔㈱ 双方向変換**: `_create_matching_variants`に`FULLTEXT_CORP_SWAPS`追加
   - `株式会社ビレッジ開発` ↔ `㈱ビレッジ開発` → マッチ成功
   - `有限会社花正` ↔ `㈲花正` → マッチ成功

2. **法人格プレフィックス除去**: `LEGAL_PREFIXES`リスト追加（10種）
   - 一般社団法人/一般財団法人/公益社団法人/公益財団法人/社会福祉法人/医療法人/学校法人/宗教法人/NPO法人/特定非営利活動法人
   - `一般社団法人刈谷労働基準協会` ↔ `刈谷労働基準協会` → マッチ成功

## 2026-02-10: [PROCESS] 25日支払欄に関するヒアリング整理（12/2, 12/8, 1/21）

### 25日支払の「欄」は削除可能だが「合計」は必須

| 質問 | 回答 |
|------|------|
| 25日支払の欄を削除してもいいか？ | **可能**（吉田さん「フォームは統一ではないので変更しても大丈夫」12/8） |
| 25日に支払うものはどう扱うのか？ | データとしては明確に分けて出力。RPAは支払日（15日/25日/末日）をキーにして抽出 |
| 支払日ごとの合計は不要か？ | **必須**。UFJへの振込データは15日/25日/末日の3本に分けてアップロード（1/21） |

### 出力形式の選択肢

| 案 | 方式 | 吉田さん反応 |
|----|------|-------------|
| A（推奨）| 同一シート内で「25日支払いブロック」行分け + 小計行 | — |
| B | 15日/25日/末日でシートまたはファイルを分ける | 「基本的にはOK」（12/2） |

**結論**: 物理的な枠線は不要だが、**支払日ごとの合計金額**がパッと見てわかる状態は必須（銀行照合用）。

---

## 2026-02-10: [CONFIG] 原本シートの決定

**決定**: RPA1_シート作成時のコピー元（原本）は、各月シート（R7.9月分等）ではなく **`RPAテンプレ`シート** を使用する。

**ソースExcelパス**: `C:\Users\masam\Desktop\KeyenceRK\ロボット作成資料\55 総合振込計上・支払伝票 12月分有_RPA_3M_20260210.xlsx`

**シート名**: `RPAテンプレ`

**理由**: 各月シートは前月データや手動修正が残っている場合があり、クリーンなテンプレートとして不適。RPAテンプレは標準化されたテンプレートとして設計されている。

**影響**: `batch_5months.py` の `create_rpa1_sheet()` で `src_sheet` を `RPAテンプレ` 固定に変更。

## 2026-02-11: [CODE] batch_5months.py v3 ExcelWriterV2統合リファクタ

**問題**: v2 batchがExcelWriterV2の案B行挿入ロジック（テスト済み）をバイパスし、自前関数（normalize_name, find_supplier_row, find_empty_row等10個）を実装 → 16件OVERFLOWエラー発生。AGENTS.mdルール（既存資産チェック）違反。

**修正方針**: batch_5months.pyから自前関数を全削除し、ExcelWriterV2.write_payment_data()に完全委託。

**ExcelWriterV2への変更（3件）**:
1. `write_payment_data()` に `target_sheet_name: str = None` + `keep_open: bool = False` パラメータ追加
   - target_sheet_name: select_sheet()をスキップし指定シートに直接書込み
   - keep_open: close_workbook()をスキップ（バッチ用）
2. `open_workbook()` にresolveガード追加 — source_excel_path == excel_pathのとき自己コピーを回避
3. `_XlwSheetAdapter` クラス名修正（NameError解消）

**batch_5months.py v3の構成**:
- 残した関数（batch固有）: create_rpa1_sheet, load_csv_data, get_payment_date, load_config, main
- 削除した自前関数（10個）: normalize_name, find_supplier_row, find_empty_row, find_cache_key, shift_caches, insert_rows_for_supplier, insert_rows_at_end, write_record, build_supplier_cache, clear_all_amounts

**変更ファイル**:
- `C:\ProgramData\RK10\Robots\55 １５日２５日末日振込エクセル作成\tools\excel_writer_v2.py`（L625,L1655,L1705,L1984）
- `scratchpad\batch_5months.py`（v3完全書き換え、315行 ← v2 468行）

## 2026-02-11: [VERIFICATION] v3バッチ実行結果

**dry-run**: Overflow 0件、Errors 0件 → OK
**実書込**: 5ヶ月 × 3支払タイプ = 15パス、Success=98件、Errors=0、Overflow=0

| 月 | 末日 | 25日 | 15日 | 合計 | 金額 |
|----|------|------|------|------|------|
| R7.9 | 22件 | 0 | 0 | 22 | 3,379,564円 |
| R7.10 | 31件 | 0 | 2件 | 33 | 2,621,777円 |
| R7.11 | 29件 | 0 | 0 | 29 | 1,152,252円 |
| R7.12 | 10件 | 0 | 0 | 10 | 1,020,610円 |
| R8.1 | 4件 | 0 | 0 | 4 | 247,500円 |

**JIDOKA-STOP**: R7.10 15日ブロックで1件発生（行5: 期待1,000,000円→実際0円）
- 原因: RPAテンプレの15日セクション行5がヘッダ行の可能性 — 要調査

**idempotency注意**: v2実行時の `log/v2_processed_LOCAL_*.csv` が残っていると全レコードスキップされる。バッチ再実行前に必ずバックアップ/削除が必要。

**出力ファイル**: `C:\Users\masam\Desktop\KeyenceRK\ロボット作成資料\55_RPA1_5M_v3_20260211.xlsx`

## 2026-02-11: [KAIZEN] AGENTS.md Critical Gates追加

**問題の根本原因（Codex分析）**: AGENTS.md 484行の中にルールが埋もれ、タスクフォーカス時に参照されない。「ルールが存在する」≠「ルールが参照される」（性弱説）。

**対策**: AGENTS.md冒頭に**Critical Gates**（10〜15行）をTrigger→Action→Evidence パターンで追加。

| Gate | Trigger | Action |
|------|---------|--------|
| Gate 1: 既存資産チェック | 新しい関数/クラス/スクリプトを書こうとしている | decisions読む + 既存コードgrep + 再利用/新規の判断理由 |
| Gate 2: ユーザー確認 | 決定・テスト済みプロセスを変えようとしている | 変更内容と理由をユーザーに提示し承認を得る |
| Gate 3: 検証手段の確認 | コードを変更しようとしている | 検証方法の確認、なければ先に作る |

**変更ファイル**: `AGENTS.md`（「最重要原則」の前にCRITICAL GATESブロック追加）

## 2026-02-11: [BUGFIX] _is_tax_row() ハードコード奇偶チェック修正（3箇所追加）

**問題**: ExcelWriterV2に`_is_tax_row()`メソッドを導入済みだったが、3箇所のハードコード奇偶チェックが残存していた。
RPAテンプレ（data_start=14, 偶数行=データ）ではこのハードコードがデータ行を全スキップし、supplier cacheが0社0行、全書込みが0 successになっていた。

**修正3箇所**:
1. Line 863: `_reset_amounts()` openpyxlパス — `row % 2 == 1` → `not self._is_tax_row(row)`
2. Line 949: `create_rpa_sheet()` クリアパス — `row % 2 == 1` → `not self._is_tax_row(row)`
3. Line 1189: `_build_supplier_cache()` — `row_num % 2 == 0` → `self._is_tax_row(row_num)`

**`_is_tax_row()`の定義** (Line 318):
```python
def _is_tax_row(self, row: int) -> bool:
    return row % 2 != self._data_start_row % 2
```
- 原本シート (data_start=17, 奇数): 偶数行=税 → `_is_tax_row(18)=True`
- RPAテンプレ (data_start=14, 偶数): 奇数行=税 → `_is_tax_row(15)=True`

**全7箇所のパリティチェック（全て`_is_tax_row()`に統一済み）**:
- Line 318: 定義
- Lines 863, 949: `not self._is_tax_row(row)` (データ行識別)
- Lines 1189, 1389, 1552, 1564: `self._is_tax_row(row)` (税行スキップ)

**変更ファイル**: `C:\ProgramData\RK10\Robots\55 １５日２５日末日振込エクセル作成\tools\excel_writer_v2.py`

## 2026-02-11: [VERIFICATION] v3バッチ再実行結果（パリティ修正後）

**前提**: パリティ修正3件適用 + idempotencyログをbackup_20260211_v3_fix/に退避

**結果**: Success: 96, Overflow: 0, Errors: 0, Unregistered: 2, Amount: 8,215,629円

| 月 | 末日 | 25日 | 15日 | 合計 | 金額 |
|----|------|------|------|------|------|
| R7.9 | 22件 | 0 | 0 | 22 | 3,379,564円 |
| R7.10 | 29件 | 0 | 2件 | 31 | 2,371,777円 |
| R7.11 | 29件 | 0 | 0 | 29 | 1,152,252円 |
| R7.12 | 10件 | 0 | 0 | 10 | 1,020,610円 |
| R8.1 | 4件 | 0 | 0 | 4 | 247,500円 |

**JIDOKA-STOP**: 名前フォーマット差異の警告のみ（〃マーク、括弧表記差）。金額不一致は0件。データ書込み自体は正常。

**出力ファイル**: `C:\Users\masam\Desktop\KeyenceRK\ロボット作成資料\55_RPA1_5M_v3_20260211.xlsx`

## 2026-02-11: [VERIFICATION] evaluate_v2.py 精度再評価（正規化改善後）

**正規化改善内容**:
- 全角英字→半角: A-Z, a-z 全52文字（以前はＴのみ対応）
- 全角記号→半角: ．→., 　→space, ＆→&
- 法人格正規化: 株式会社→㈱, 有限会社→㈲, 合同会社→(同), 一般社団法人/一般財団法人→削除
- 括弧+内容除去: （ボディマン）→削除（全角→半角変換後に正規表現で除去）
- 全空白削除 + lowercase化
...
| シート | マッチ率 | 詳細 |
|--------|---------|------|
| R7.9月分 | - | GT/RPA1共にデータなし（スキップ） |
| R7.10月分 | 89.5% (34/38) | Unmatched GT: 4件 |
| R7.11月分 | 88.5% (23/26) | Unmatched GT: 3件 |
| R7.12月分 | 0.0% (0/2) | GT=末日データのみ、RPA1=25日データのみ（データスコープ不一致） |
| **Overall** | **86.4% (57/66)** | 前回69.7%から+16.7pt改善 |

**Unmatched原因（代表例）**:
- ㈱ビレッジ開発: CSVにデータなし（楽楽精算で該当月に未承認 or 別会社コード）
- ジャパンメディアシステム㈱: 同上
- 微妙な名前差異: 正規化後も一致しないケース（特殊文字、略称差）

**R7.12月分の0%について**: GT（RPA_R7.12月分）にはBlock2末日のデータのみ（2件）、RPA1にはBlock1の25日データが記録されている。データスコープの不一致であり、コードバグではない。

**変更ファイル**: `scratchpad\evaluate_v2.py`（normalize()関数の強化）

## 2026-02-11: [CONFIG] RPAテンプレ正本ソースの確定（Supersedes: v3/v5のSOURCE設定）

### 決定

RPAテンプレの正本（canonical source）を以下に確定:

**ファイル**: `C:\ProgramData\RK10\Robots\55 １５日２５日末日振込エクセル作成\docs\55 総合振込計上・支払伝票_RPA用原本.xlsx`

**RPAテンプレシート構造**:
- K13=支払日, L13=支払日計（K,L列あり）
- AutoFilter: $A$13:$L$723
- Merges: B14:B15, I14:I15
- Used range: 1-734行, 1-12列(A-L)

### 運用ルール

| 環境 | 手順 |
|------|------|
| LOCAL | RPA用原本.xlsx のRPAテンプレシートをそのまま使用（read-only source） |
| PROD | RPA用原本のRPAテンプレシートを本番原本に追加してから実行 |

### Supersedes
- v5設定: デスクトップ `55 総合振込計上・支払伝票 12月分有.xlsx` をソースとしていた
- v3設定: `55 総合振込計上・支払伝票 12月分有_RPA_3M_20260210.xlsx` をソースとしていた
- [CONFIG] 原本シートの決定 (2026-02-10): ソースExcelパスをデスクトップファイルに指定していた

### v5バッチ検証結果（正本ソース変更前のデスクトップファイル版）

| 月 | ok | new | amt |
|----|-----|-----|-----|
| R7.9 | 35 | 1 | 3,448,362円 |
| R7.10 | 34 | 1 | 3,515,577円 |
| R7.11 | 24 | 0 | 1,109,752円 |
| R7.12 | 3 | 0 | 141,938円 |
| R8.1 | 0 | 0 | 0円 |
| **GRAND** | **96** | **2** | **8,215,629円** |

Overflow=0, Errors=0。K,L列・AutoFilter・Merges全て保持確認済み。

**変更ファイル**: `scratchpad\batch_5months.py`（SOURCE_EXCEL を正本パスに更新）

## 2026-02-11: [BUGFIX] スクレイパーナビゲーションバグ修正 + 再スクレイピング検証

### 問題
`rakuraku_scraper.py` の `navigate_to_payment_requests()` がフレーム内リンククリック方式で、支払依頼**入力画面**（sapShihairaiDenpyo）に遷移していた。正しくは支払依頼**一覧画面**（sapShihairaiZenbumonKensaku）に遷移すべき。

**根本原因**: `frame.query_selector('a:has-text("支払依頼")')` が同名リンク2つ（入力画面・一覧画面）のうち入力画面のリンクを先にマッチしていた。また、閲覧タブのクリックは `href=""` のJSトグルで実際のナビゲーションが発生しないケースがあった。

### 修正
`navigate_to_payment_requests()` をフレーム走査方式から**直接URL遷移**に変更:
```python
target_url = f"{base}/sapShihairaiZenbumonKensaku/initializeView"
self.page.goto(target_url)
```
- コード量: 84行 → 39行
- リダイレクト検出（ログイン画面へのリダイレクト = 遷移失敗）を追加

### 再スクレイピング結果
| 月 | 修正前 | 修正後 | 備考 |
|----|--------|--------|------|
| R7.12（2025年12月）| 末日=0件 | **末日=29件**（支払予定日=2025/12/30） | データは楽楽精算に存在していた |
| R8.1（2026年1月）| 末日=0件 | **末日=0件**（正当） | ユーザー確認済み: 1月末日データは未登録 |

### 教訓
- **「データがない」という結論を出す前に、ツール側の問題を疑う**（Gate 5: 矛盾チェック）
- 12月末日データは過去6ヶ月間ずっと扱ってきた既知事実 → 「ない」は既知事実と矛盾 → ツールを先に疑うべきだった
- CSVを二次ソースとして確認するのではなく、一次ソース（楽楽精算）に直接アクセスする

**変更ファイル**: `C:\ProgramData\RK10\Robots\55 １５日２５日末日振込エクセル作成\tools\rakuraku_scraper.py`

## 2026-02-11: [PROCESS] 構造的検証プロトコル導入（性弱説の仕組み化）

### 背景
ルールをAGENTS.md/CLAUDE.md/MEMORY.mdに何度書いても「楽な方に行く」パターンが再発。
「ルールが存在すること」と「ルールに従うこと」は別問題 — 性弱説はAIにも適用される。

### 対策（4つの構造変更）
1. **Gate 4: ソース明記** — 事実主張の前に一次ソース（コマンド/パス/URL）を必須化
2. **Gate 5: 矛盾チェック** — 否定的主張の前に既知事実との突合を必須化
3. **自律度制御** — 調査・分析・報告タスクはLOW自律度（1結論ごとにユーザー確認）
4. **構造的検証プロトコル** — レポート形式・ネガティブクレームゲート・ショートカット防止チェックリスト
