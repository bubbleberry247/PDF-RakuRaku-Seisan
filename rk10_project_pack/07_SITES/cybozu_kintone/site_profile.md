# Site Profile: サイボウズ（kintone/Garoon） (cybozu_kintone)

## 1. 対象範囲
- Base URL: (要記入: 貴社cybozuドメインURL)
- 主要画面: (要追記)
- 目的（自動化）: (要追記)
...
## 3. 認証・セッション
- ログイン方式: SSO/MFAの可能性(要確認)
- セッション維持の注意: (要確認)
- ブロック要因（CAPTCHA等）: (要確認)

## 4. 推奨ツール選定（壊れにくさ優先）
- Primary: API優先(可能なら) / Playwright(要検証)
- Fallback: RK10
- 選定理由: 設定差が大きい。可能ならAPI/エクスポート導線を優先。UIは待機・権限・画面遷移の影響が出やすい。
