...
REM ============================================================
REM  Autonomous Codex Loop
REM  Usage: run_codex_loop.bat [max_iterations]
...
set "PROMPT_FILE=%PROJECT_DIR%\AGENT_PROMPT_CODEX.md"
...
set "LOG_FILE=%LOG_DIR%\codex_run_%TIMESTAMP%.log"
...
REM Codex uses --full-auto for autonomous mode, --sandbox workspace-write for file access
codex exec --model gpt-5.3-codex --sandbox workspace-write --full-auto "Follow the instructions in AGENT_PROMPT_CODEX.md. This is iteration %ITER% of %MAX_ITER%." >> "%LOG_FILE%" 2>&1

if errorlevel 1 (
    echo [ERROR] Codex exited with error. Check log.
    echo [ERROR] Codex exited with error at iteration %ITER% >> "%LOG_FILE%"
    goto :end
)
