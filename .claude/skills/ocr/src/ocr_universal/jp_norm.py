...
# ---------------------------------------------------------------------------
# 簡体字→日本語漢字（PaddleOCR誤出力の補正）
# ---------------------------------------------------------------------------

_SIMPLIFIED_TO_JAPANESE: dict[str, str] = {
    "费": "費",  # 消费→消費
    "单": "単",
    "额": "額",  # 金额→金額
    "进": "進",
...
_SIMPLIFIED_TRANS = str.maketrans(_SIMPLIFIED_TO_JAPANESE)


def normalize_simplified_chinese(text: str) -> str:
    """簡体字を日本語漢字に変換する。

    PaddleOCR が日本語請求書のラベル（摘要・単位・金額等）を簡体字で出力
    する問題への対策。固定ラベルにのみ適用し、vendor_name や固有名詞には
    使わないこと。
    """
    return text.translate(_SIMPLIFIED_TRANS)
