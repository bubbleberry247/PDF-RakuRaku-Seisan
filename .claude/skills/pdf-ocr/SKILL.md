...
## ⛔ 実装着手前ゲート（必須 — これを満たすまでコードを書いてはならない）

このスキルが呼ばれたとき、以下を**実行してから**着手すること：

### Step 1: 実装ファイルを確認（Globで一覧取得）
```
Glob: external-repos/my-claude-skills/pdf-ocr/templates/**/*.py
```
取得したファイル名を**必ずユーザーに提示**する。

### Step 2: 使用する関数・クラスを列挙
確認した実装ファイルから、今回使う関数・クラス名を列挙する。

### Step 3: 宣言してから着手
「`pdf_preprocess.py` の `PDFPreprocessor.preprocess()` を使って実装します」など、
**どのファイルのどの関数を使うか**を明示してからコードを書く。

> **上記3ステップなしにコードを書いた場合は即停止。**
> SKILL.mdの説明文だけ読んで「スキルを使った」と判断するのは禁止。
