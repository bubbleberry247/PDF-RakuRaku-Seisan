...
- **2行1セット**: 偶数行=本体（RPA書込対象）、奇数行=税（RPA書込禁止）。データ開始は行14。
- **列構成（A～L、12列）**: AutoFilter範囲 `$A$13:$L$723`

| 列 | ヘッダー | 本体行（偶数） | 税行（奇数） | RPA書込 |
|---|---------|---------------|-------------|---------|
| A | 業者番号 | 値 | 空（マージ） | テンプレのみ |
| B | 支払先 | 値 | 空（マージ） | 未登録のみ |
| C | 摘要 | 値 | **数式**（月名自動） | 本体のみ |
| D | 店 | 値 | 値 | **禁止**（手動） |
| E | 科目 | 値 | **数式**（仮払消費税/-） | 本体のみ |
| F | 科目コード | 値 | **数式**（1600/-） | 本体のみ |
| G | 税 | **数式**（税区分） | **数式**（税区分） | **禁止**（数式） |
| H | 請求額 | **数式**（税抜金額） | **数式**（消費税額） | **禁止**（数式） |
| I | 支払額 | 税込金額 | 空（マージ） | **書込** |
| J | 支払先計 | 値 or SUM数式 | 空（マージ） | **書込**（J=I） |
| K | 支払日 | 値 | 空 | **書込** |
| L | 支払日計 | 値 | 空 | **書込** |

- **H列数式（税計算の核心）**:
  - 本体行: `=ROUNDDOWN(I14-H15, 0)` → 税抜金額（I列 - 消費税）
  - 税行: `=ROUNDDOWN(I14*10/110, 0)` → 消費税額（10%）
  - **8%軽減税率行あり**: `=ROUNDDOWN(I26*8/110, 0)` — 食料品等スロット
- **G列数式（税区分自動判定）**: `=IF(E15="-",0,IF(ISERROR(SEARCH("*非課税*",E14)),3,1))` → 3=課税, 1=非課税, 0=税行なし
- **J列SUM数式**: 同一支払先に複数明細がある場合 `=SUM(I24:I25)` 等（16箇所確認）
...
├── docs\
│   ├── 55 総合振込計上・支払伝票.xlsx           (v2 template)
│   └── 55 総合振込計上・支払伝票_RPA用原本.xlsx  (★RPAテンプレ正本)
└── work\                                        (作業出力フォルダ)
```

## 6. RPAテンプレ正本 & PROD Paths

**RPAテンプレ正本（2026-02-11確定）**:
- **ファイル**: `C:\ProgramData\RK10\Robots\55 １５日２５日末日振込エクセル作成\docs\55 総合振込計上・支払伝票_RPA用原本.xlsx`
- **RPAテンプレシート**: K13=支払日, L13=支払日計, AutoFilter $A$13:$L$723, Merges B14:B15/I14:I15
- **LOCAL**: このファイルのRPAテンプレシートをそのまま使用
- **PROD**: RPAテンプレシートを本番原本に追加してから実行
- **作業出力**: `work\` フォルダに保存（`docs\` は原本保管のみ、作業禁止）
