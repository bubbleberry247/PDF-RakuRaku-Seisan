...
### 2026-05-21
- **RK10 RKS改修ゲート**: `.rks` はZIP/C#として正常でも、RK10独自の `ReactiveSyntaxTree` 変換で開けない場合がある。RKS直接編集は原則避け、業務ロジックは外部BAT/Python/CSVへ寄せる。
- **RKS変更の合格条件**: 1変更ごとに別名保存し、`try/catch/finally`増加なし・相対パスなし・未定義関数なし・旧BAT名残りなしを静的確認した上で、必ずRK10で開けることを最終ゲートにする。`zip OK` / `py_compile OK` / C#構文OKだけでは完了扱いにしない。
- **シナリオ43失敗知見**: v85/v85-1でZIP内 `Program.cs` を直接合成し、RK10で開く確認を最終ゲート化しなかったため、ユーザー実行時にRKS破損・相対パス問題を検出した。以後、RKS本体の直接生成より、RK10エディタで開ける母体と外部スクリプト側改修を優先する。
