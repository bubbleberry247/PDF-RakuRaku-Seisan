...
- **Trigger**: 新しい関数/クラス/スクリプトを書こうとしている、または既存コードを削除/変更しようとしている
...
- **Chesterton's Fence**: 「なぜこのコードが存在するか」を説明できるまで、削除・変更してはいけない。理由が不明なら、まずgit blame/ログで経緯を調べる
...
### Anti-rationalization Table（言い訳封じ — 性弱説④飛ばす構造的対策）

Gateを「飛ばす理由」を事前に潰す。各言い訳に実際の失敗事例を紐付ける（inspired by Google SWE / Addy Osmani agent-skills）。

| 言い訳 | 反論 | 実証事例 |
|--------|------|----------|
| 「小さい変更だからテスト不要」 | 小さい変更が最もリグレッションを起こす | archi-16w @32: ナビボタン非表示→Home/中断ボタンまで消失 |
| 「既存コード調べるより書いた方が早い」 | 既存テスト済みコードのバイパスは必ず負債になる | S55 batch_5months.py: excel_writer_v2.pyを無視→16件OVERFLOW |
| 「動いたからOK」 | 「動いた」≠「検証できた」。Exit Criteria未達は未完了 | OCR Fix 5: confidence閾値追加→他ファイルで金額欠損リグレッション |
| 「ログ見れば分かる」 | 二次ソースで判断は誤報告の温床。Gate 4: 一次ソース必須 | CSV「148件」で分析→実際は「一般経費のみ35件」が正解 |
| 「たぶんないと思う」 | 推測で否定的主張→ユーザーの時間を浪費。Gate 5必須 | スクレイパー0件→「データなし」報告→実はナビゲーションバグ |
| 「仕様書は後で書く」 | 後で書かれた仕様書はない。実装前に書く | 仕様駆動開発原則（2026-01-30制定） |
| 「コード読まなくても数値で分かる」 | 表面的な数値で判断→仕様を誤認。Code-First Verification必須 | eval_accuracy.py: month-1を「バグ？」→実は会計月ズレの正仕様 |
| 「削除しても問題ないだろう」 | Chesterton's Fence: 存在理由を説明できるまで触るな | Gate 1違反の汎用パターン |

**運用**: 新しい失敗事例が発生したら、このテーブルに追記する（append-only）。
...
### 仕様ゲート（Spec Gate — 仕様駆動開発の構造的強制）
- **新機能・大きな変更の場合**: 仕様（要件/設計/完了条件）をドキュメント化し、ユーザー承認を得てから実装に入る
- **ドキュメント先行**: 仕様書 → レビュー承認 → 実装（この順序を逆にしない）
- **最小仕様**: 小さなタスクでも「何を/なぜ/完了条件」の3点は明示する
- **Gate違反の検知**: 仕様なしで実装が始まっている場合はSTOP。先に仕様を書く
...
問題が発生したら、この順序で進める（Google SWE inspired）:

1. **Reproduce** — 問題を再現する。再現手順を明文化する
2. **Localize** — 原因箇所を特定する。二分探索・ログ・git bisect等
3. **Reduce** — 最小再現ケースに絞る。無関係な要素を除去
4. **Fix** — 根本原因を修正する（表面的なワークアラウンド禁止）
5. **Guard** — 再発防止テストを追加する。同じ問題が二度と起きない構造にする

### 基本原則：二度と同じ問題を起こさない
1. **問題の深掘り（5 Whys）**
2. **再発防止策の策定（仕組みで防ぐ）**— Guard ステップで具体化
3. **ドキュメント更新**
   - 恒常ルール → `Project Memory`
   - 決定/事実 → `plans/decisions.md`
   - 言い訳パターン → Anti-rationalization Table に追記
4. **横展開（他シナリオ影響確認）**
